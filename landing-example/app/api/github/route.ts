...
export async function GET() {
  try {
    ...
    const [repoRes, releasesRes, commitsRes, contributorsRes] = await Promise.all([
      fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}`, { 
        headers, 
        next: { revalidate: 3600 } 
      }),
      fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/releases?per_page=10`, { 
        headers, 
        next: { revalidate: 3600 } 
      }),
      ...
      fetch(`https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contributors?per_page=100&anon=true`, { 
        headers, 
        next: { revalidate: 3600 } 
      }),
    ])
    ...
    const releases: GitHubRelease[] = releasesRes.ok ? await releasesRes.json() : []
    ...
    const oldestFetched = allCommits.length > 0 
    ...
    if (oldestFetched > ninetyDaysAgo) {
      ...
      while (page <= 4) {
        ...
        const oldest = new Date(pageCommits[pageCommits.length - 1].commit.author.date)
    ...
    for (let i = 89; i >= 0; i--) {
      ...
      commitsByDate.set(dateStr, { date: dateStr, commits: 0, details: [] })
    }
    ...
    const versionUrl = latestRelease?.html_url || `https://github.com/${REPO_OWNER}/${REPO_NAME}`

    const latestCommitDate = allCommits[0]
      ? new Date(allCommits[0].commit.author.date).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
        })
