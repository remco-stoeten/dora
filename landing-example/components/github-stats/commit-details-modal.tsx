...
import { X, GitCommit, User, Clock, FileCode, Plus, Minus, ExternalLink } from "lucide-react"
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a1a1a]">
          <div>
            <h2 id="modal-title" className="text-[#9a9a9a] font-medium">
              {data.date}
            </h2>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-[#9a9a9a] text-sm truncate flex-1">{commit.message}</p>
                  ...
                  <div className="flex items-center gap-3 mt-1.5 text-[10px] text-[#4a4a4a]">
                    ...
                    <span className="flex items-center gap-1">
                      {!commit.authorAvatar && <User className="w-2.5 h-2.5" />}
                      {commit.author}
                    </span>
                    <span className="flex items-center gap-1">
                    ...
                    <code className="font-mono text-[#3a3a3a]">{commit.sha}</code>
                  </div>
                </div>
              </div>

              {/* Stats (if available) */}
              {(commit.additions !== undefined || commit.files) && (
