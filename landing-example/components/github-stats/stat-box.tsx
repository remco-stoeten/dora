...
export function StatBoxGroup({ children, className = "", noBorder = false }: StatBoxGroupProps) {
  return (
    <div className={`px-5 py-4 ${!noBorder ? "border-r border-[#1a1a1a]" : ""} ${className}`}>
      {children}
    </div>
  )
}
...
export function StatItem({ icon: Icon, label, children, href, small = false }: StatItemProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`text-[#8a8a8a] ${small ? "text-xs" : "text-sm"}`}>{children}</div>
    </Wrapper>
  )
}
