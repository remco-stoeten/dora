...
import { Tag, Calendar, GitCommit, Clock, Star, Loader2, Download, Check, Copy } from "lucide-react"
import { CommitGraph, type CommitDataPoint, type CommitDetail } from "./commit-graph"
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-1.5 rounded hover:bg-[#2a2a2a] text-[#4a4a4a] hover:text-[#8a8a8a] transition-colors"
      title="Copy command"
    >
...
function TabIndicator({ activeRect }: { activeRect: DOMRect | null }) {
  ...
  return (
    <div
      className="absolute bg-[#1a1a1a] border border-[#2a2a2a] rounded-md transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]"
      style={{
...
export function GitHubStats({ accentColor = "#22c55e" }: GitHubStatsProps) {
  const { data, error, isLoading } = useSWR<GitHubStatsData>("/api/github", fetcher, {
  ...
  const handleHoverChange = (index: number | null, position?: { x: number; y: number }) => {
  ...
  const activePackage = packages.find(p => p.platform === activeInstall) ?? packages[0]

  return (
    <>
      <div className="w-full bg-[#0a0a0a] px-6 sm:px-8 py-12">
        <div className="border border-[#1a1a1a] overflow-hidden">
          {/* Top row: Info + Commits */}
          <div className="flex">
            {/* Left info section: Version + Timeline */}
            <div className="flex-shrink-0 w-64 border-r border-[#1a1a1a]">
              {/* Version */}
              <a 
                href={versionUrl}
                target="_blank"
              ...
              >
                ...
                <div className="text-[#9a9a9a] text-lg font-mono font-medium">{version}</div>
              </a>
              ...
              <div className="px-5 py-4">
                <div className="flex items-center gap-4 text-[11px]">
                  <div>
                    ...
                    <div className="text-[#7a7a7a]">{startedAt}</div>
                  </div>
                  <div className="w-px h-8 bg-[#1a1a1a]" />
                  <a 
                    href={latestCommitSha ? `${sourceUrl}/commit/${latestCommitSha}` : undefined}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  >
                    ...
                    <div className="text-[#7a7a7a]">{latestCommitAt}</div>
                  </a>
                </div>
            ...
            <div
              ...
              className="flex-1 px-6 py-4 relative group overflow-visible min-h-[120px]"
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <GraphTooltip
                    data={hoveredIndex !== null ? commitData[hoveredIndex] : null}
              ...
              <div className="relative z-20 pointer-events-none">
                ...
                <div className="text-[10px] text-[#3a3a3a] mt-1 flex items-center gap-2">
          ...
          <div className="border-t border-[#1a1a1a] px-6 py-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div ref={tabsContainerRef} className="relative flex items-center gap-1">
                <TabIndicator activeRect={indicatorRect} />
                {packages.map((pkg) => (
                  <button
                    key={pkg.platform}
                    ref={(el) => {
                      if (el) tabRefs.current.set(pkg.platform, el)
                    }}
                    onClick={() => setActiveInstall(pkg.platform)}
                    className={`relative z-10 flex items-center gap-2 px-3 py-2 rounded-md text-xs font-medium transition-colors duration-200 ${
                    ...
                    title={pkg.name}
                  >
                    <PlatformIcon platform={pkg.platform} className="w-4 h-4" />
                    <span>{pkg.name}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="flex-1 max-w-xl">
                {activePackage && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 bg-[#0d0d0d] border border-[#1a1a1a] rounded-md px-4 py-2.5 group/cmd hover:border-[#2a2a2a] transition-all duration-300"
                  >
                    <code className="text-sm text-[#6a6a6a] font-mono flex-1 truncate group-hover/cmd:text-[#8a8a8a] transition-colors">
                      {activePackage.command || `Download from ${activePackage.name}`}
                    </code>
                    {activePackage.command && <CopyButton text={activePackage.command} />}
                  </a>
                )}
              </div>

              {/* Downloads indicator */}
              {activePackage?.downloads !== undefined && activePackage.downloads > 0 && (
                <span className="text-[10px] text-[#3a3a3a] whitespace-nowrap">
                  {formatDownloads(activePackage.downloads)} downloads
                </span>
              )}
            </div>
          </div>
      ...
      <CommitDetailsModal
        ...
        data={selectedDayIndex !== null && commitData.length > 0 ? commitData[selectedDayIndex] : null}
        accentColor={accentColor}
        repoUrl={sourceUrl}
      />
    </>
