...
export function CodeEditor({ value, onChange, onExecute, onSave, onModeChange, isExecuting, tables }: Props) {
  ...
  const handleEditorDidMount: OnMount = function (editor, monaco) {
    ...
    completionProviderRef.current = monaco.languages.registerCompletionItemProvider("typescript", {
      triggerCharacters: [".", "(", ",", " "],
      exclusive: true,
      provideCompletionItems: async function (model, position): Promise<SuggestList | undefined> {
            ...
            return buildSuggestions(range, [
              {
                label: "where",
                kind: monaco.languages.CompletionItemKind.Property,
                insertText: "where: (${1:table}, { eq }) => eq(${1:table}.${2:id}, ${3:value}),",
                insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
              ...
              },
              {
                label: "orderBy",
                kind: monaco.languages.CompletionItemKind.Property,
                insertText: "orderBy: (${1:table}, { desc }) => [desc(${1:table}.${2:id})],",
                insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
