...
import { SUPABASE_MOCK } from '../_shared/mock-provider-data'
...
export function SupabaseConnectFlow({ onComplete }: Props) {
	...
	if (!isTauri) {
		return <MockProviderConnectFlow config={SUPABASE_MOCK} onComplete={onComplete} />
	}
...
function SupabaseConnectFlowInner({ onComplete }: Props) {
	...
	useEffect(
		function prefillStoredPassword() {
			...
			{!isConnected ? (
				<div className='space-y-3'>
					...
					<p className='text-xs text-muted-foreground/70'>
						Opens your browser to authorize Dora. Access is encrypted and stored on this
						device only.
					</p>
					...
					{showTokenFallback ? (
						<div className='space-y-2 border-t border-border/50 pt-3'>
							...
							<p className='text-xs text-muted-foreground/70'>
								The token is validated, then encrypted and stored on this device
								only.
							</p>
						</div>
					) : (
