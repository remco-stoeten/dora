...
import { TURSO_MOCK } from '../_shared/mock-provider-data'
...
export function TursoConnectFlow({ onComplete }: Props) {
	...
	if (!isTauri) {
		return <MockProviderConnectFlow config={TURSO_MOCK} onComplete={onComplete} />
	}
...
function TursoConnectFlowInner({ onComplete }: Props) {
	...
	useEffect(
		function detectTursoCliOnMount() {
			void probeCli()
		},
		[probeCli]
	)
	...
	async function handleSignIn() {
		...
		try {
			...
			toast(loggedIn ? 'Signed in to Turso' : "Sign-in didn't complete", {
							...
							{cliLoggedIn ? (
								<>
									...
									>
										...
										{isMinting
											? 'Minting via Turso CLI…'
											: 'Mint a token with the Turso CLI'}
									</Button>
									<p className='text-xs text-muted-foreground/70'>
										Dora runs the Turso CLI to mint a token for you — no
										copy-paste needed.
									</p>
								</>
							) : (
						...
						<div className='space-y-2'>
							...
							<p className='text-xs text-muted-foreground/70'>
								...
								<span className='font-mono text-[11px]'>
									app.turso.tech/settings/tokens
								</span>{' '}
